...
sellers_tbl      <- read_csv(file = "DS_business_case_01/00_data/01_e-commerce/01_raw_data/olist_sellers_dataset.csv")
# 3.0 Examining Data ----
...
order_items_joined2_tbl  <- order_items_tbl %>%
  left_join(orders_tbl) %>%
  left_join(products_tbl) %>%
  left_join(sellers_tbl)
# Examine the results with glimpse()
order_items_joined2_tbl %>% glimpse()
# 5.0 Wrangling Data ----
#take a look at 1 column
order_items_joined2_tbl$seller.location

# All actions are chained with the pipe already. You can perform each step separately and use glimpse() or View() to validate your code. Store the result in a variable at the end of the steps.
order_items_wrangled2_tbl <- order_items_joined2_tbl %>%
  ...
  bind_cols(order_items_joined2_tbl %>% select(product.id)) %>% 
  
# 6.0 Business Insights ----
# 6.1 Sales by Year ----

# Step 1 - Manipulate
State_tbl <- order_items_wrangled2_tbl %>%
...
State_tbl %>%
  ...
  ggplot(aes(x = state, y = summarize)) +
